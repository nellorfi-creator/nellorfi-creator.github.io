...
export default function MachineDetail({ machine, prev, next }: Props) {
  ...
  return (
    <main className={styles.page} id="top">
      <header className={styles.nav}>
        ...
        <nav className={menuOpen ? styles.open : ""} aria-label="Menu scheda macchina">
          <Link className={styles.siteLink} href="/macchine/gambe" onClick={() => setMenuOpen(false)}>
            ← Gambe
          </Link>
      ...
      <div className={styles.body}>
        ...
        <section className={styles.leadBlock}>
          <small>02 · ALLENAMENTO GAMBE</small>
          <h2>
            COME SI USA
            <br />
            <em>IN SALA.</em>
      ...
      <nav className={styles.pager} aria-label="Altre macchine gambe">
        <Link href={`/macchine/gambe/${prev.id}`}>
          <span>← Precedente</span>
          <strong>{prev.name}</strong>
        </Link>
        <Link href="/macchine/gambe" className={styles.pagerHome}>
          Tutte le gambe
        </Link>
        <Link href={`/macchine/gambe/${next.id}`} className={styles.pagerNext}>
