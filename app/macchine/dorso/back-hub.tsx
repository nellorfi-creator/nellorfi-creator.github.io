...
export default function BackHub() {
  const [menuOpen, setMenuOpen] = useState(false);
  const [zonesOpen, setZonesOpen] = useState(false);
  const loop = [...backMachines, ...backMachines];
  ...
  return (
    <main className={styles.page} id="top">
      ...
      <header className={styles.nav}>
        ...
        <nav className={menuOpen ? styles.open : ""} aria-label="Menu allenamento dorso">
      ...
      <section className={styles.hero}>
        ...
        <SiteImage className={styles.heroAthlete} src="/photos/athletes/dorso-athlete-hero.webp" alt="Atleta dedicato all’allenamento del dorso" />
        <div className={styles.heroContent}>
          <p className={styles.eyebrow}><span></span> {backZone.eyebrow}</p>
          <h1>{backZone.heroTitle[0]}<br />{backZone.heroTitle[1]}<br /><em>{backZone.heroTitle[2]}</em></h1>
          <p>{backZone.heroLead}</p>
          <div className={styles.heroMeta}><div><strong>{String(backMachines.length).padStart(2, "0")}</strong><span>Macchine in area</span></div><div><strong>01</strong><span>Passa · pausa · click</span></div></div>
        </div>
        <div className={styles.marqueeWrap} aria-label="Galleria macchine dorso">
          ...
          <div className={styles.marquee}><div className={styles.marqueeTrack}>
            {loop.map((machine, index) => (
              <Link key={`${machine.id}-${index}`} href={`/macchine/dorso/${machine.id}`} className={styles.marqueeCard} aria-label={`${machine.name} · ${machine.brand}`}>
      ...
      <section className={styles.manifesto} id="qualita">
        <div className={styles.manifestoCopy}><p className={styles.eyebrow}><span></span> Perché Revenge</p><h2>{backZone.manifestoTitle[0]}<br /><em>{backZone.manifestoTitle[1]}</em></h2></div>
        <div className={styles.manifestoText}>{backZone.manifesto.map((paragraph) => <p key={paragraph.slice(0, 40)}>{paragraph}</p>)}</div>
      </section>

      <section className={styles.index} id="elenco">
        <div className={styles.sectionIntro}><p className={styles.eyebrow}><span></span> Catalogo completo</p><h2>SCEGLI LA<br /><em>MACCHINA.</em></h2><p>Dieci postazioni per allenare il dorso da traiettorie diverse. Apri ogni scheda per scoprire funzione, muscoli coinvolti, impostazione, programmazione ed errori comuni.</p></div>
        <div className={styles.grid}>{backMachines.map((machine) => (
          <Link key={machine.id} href={`/macchine/dorso/${machine.id}`} className={styles.gridCard}>
      ...
      <section className={styles.cta}><p className={styles.eyebrow}><span></span> In sala</p><h2>LEGGERE AIUTA.<br /><em>TIRARE CAMBIA TUTTO.</em></h2><p>Vieni a Revenge Gym e costruisci la tua progressione dorso con lo staff.</p><Link href="/?skipIntro=1#contatti" className={styles.primary}>Chiedi info <span>↗</span></Link></section>
      <footer className={styles.footer}><Link href="/?skipIntro=1#home" className={styles.logo} aria-label="Revenge Gym home"><SiteImage src="/brand/revenge-gym-logo.png" alt="Revenge Gym" /></Link><p>Allenamento · Dorso · Ladispoli</p><p className={styles.footerLegal}><span>© 2026 Revenge Gym</span><span className={styles.byNello} style={{ textTransform: "none" }}>© by nello 2026</span></p><a href="#top" className={styles.backTop} aria-label="Torna su">↑</a></footer>
    </main>
  );
}
