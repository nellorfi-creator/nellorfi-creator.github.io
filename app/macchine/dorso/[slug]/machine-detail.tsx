...
export default function BackMachineDetail({ machine, prev, next }: Props) {
  ...
  return (
    <main className={styles.page} id="top">
      ...
      <div className={styles.body}>
        ...
        <section className={styles.leadBlock}><small>02 · ALLENAMENTO DORSO</small><h2>COME SI USA<br /><em>IN SALA.</em></h2>{machine.lead.map((paragraph) => <p key={paragraph.slice(0, 48)}>{paragraph}</p>)}</section>
      ...
      <nav className={styles.pager} aria-label="Altre macchine dorso">
        <Link href={`/macchine/dorso/${prev.id}`}><span>← Precedente</span><strong>{prev.name}</strong></Link>
