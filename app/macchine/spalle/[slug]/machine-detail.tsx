...
export default function ShoulderMachineDetail({ machine, prev, next }: Props) {
  ...
  return <main className={styles.page} id="top">
    ...
    <div className={styles.body}>
      ...
      <div className={styles.grid2}><section><small>03 · MUSCOLI</small><h3>COSA LAVORA<br />DAVVERO.</h3><div className={styles.dual}><div><b>PRINCIPALI</b>{machine.primaryMuscles.map((item) => <p key={item}>{item}</p>)}</div><div><b>IN ASSISTENZA</b>{machine.secondaryMuscles.map((item) => <p key={item}>{item}</p>)}</div></div></section><section className={styles.dark}><small>04 · RUOLO</small><h3>DOVE SI<br />INSERISCE.</h3><ul>{machine.trainingRole.map((item) => <li key={item}>{item}</li>)}</ul></section></div>
      <div className={styles.grid2}><section className={styles.accent}><small>05 · CUE TECNICI</small><h3>ESECUZIONE<br />PROFESSIONALE.</h3><ul>{machine.cues.map((item) => <li key={item}>{item}</li>)}</ul></section><section><small>06 · PROGRAMMAZIONE</small><h3>COME USARLA<br />NELLA SETTIMANA.</h3><ul>{machine.programming.map((item) => <li key={item}>{item}</li>)}</ul></section></div>
