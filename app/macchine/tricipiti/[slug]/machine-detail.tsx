...
export default function TricepsMachineDetail({ machine, prev, next }: Props) {
  ...
  return (
    <main className={styles.page} id="top">
      ...
      <header className={styles.nav}>
        ...
        <nav className={menuOpen ? styles.open : ""} aria-label="Menu scheda macchina tricipiti">
          <Link className={styles.siteLink} href="/macchine/tricipiti" onClick={() => setMenuOpen(false)}>
            ← Tricipiti
          </Link>
      ...
      <div className={styles.body}>
        ...
        <section className={styles.leadBlock}>
          <small>02 · ALLENAMENTO TRICIPITI</small>
          <h2>
            COME SI USA
            <br />
            <em>IN SALA.</em>
      ...
      <nav className={styles.pager} aria-label="Altre macchine tricipiti">
        <Link href={`/macchine/tricipiti/${prev.id}`}>
          <span>← Precedente</span>
          <strong>{prev.name}</strong>
        </Link>
        <Link href="/macchine/tricipiti" className={styles.pagerHome}>
          Tutti i tricipiti
        </Link>
        <Link href={`/macchine/tricipiti/${next.id}`} className={styles.pagerNext}>
