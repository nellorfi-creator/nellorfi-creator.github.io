...
export default function TricepsHub() {
  const [menuOpen, setMenuOpen] = useState(false);
  const [zonesOpen, setZonesOpen] = useState(false);
  const loop = [...tricepsMachines, ...tricepsMachines];
  return (
    <main className={styles.page} id="top">
      ...
      <header className={styles.nav}>
        ...
        <nav className={menuOpen ? styles.open : ""} aria-label="Menu allenamento tricipiti">
      ...
      <section className={styles.hero}>
        ...
        <SiteImage className={styles.heroAthlete} src="/photos/athletes/tricipiti-athlete-hero.webp" alt="Atleta dedicato all’allenamento dei tricipiti" />
        <div className={styles.heroContent}>
          <p className={styles.eyebrow}>
            <span></span> {tricepsZone.eyebrow}
          </p>
          <h1>
            {tricepsZone.heroTitle[0]}
            <br />
            {tricepsZone.heroTitle[1]}
            <br />
            <em>{tricepsZone.heroTitle[2]}</em>
          </h1>
          <p>{tricepsZone.heroLead}</p>
          <div className={styles.heroMeta}>
            <div>
              <strong>{String(tricepsMachines.length).padStart(2, "0")}</strong>
        ...
        <div className={styles.marqueeWrap} aria-label="Galleria macchine tricipiti">
          ...
          <div className={styles.marquee}>
            <div className={styles.marqueeTrack}>
              {loop.map((machine, index) => (
                <Link key={`${machine.id}-${index}`} href={`/macchine/tricipiti/${machine.id}`} className={styles.marqueeCard} aria-label={`${machine.name} · ${machine.brand}`}>
      ...
      <section className={styles.manifesto} id="qualita">
        <div className={styles.manifestoCopy}>
          ...
          <h2>
            {tricepsZone.manifestoTitle[0]}
            <br />
            <em>{tricepsZone.manifestoTitle[1]}</em>
          </h2>
        </div>
        <div className={styles.manifestoText}>
          {tricepsZone.manifesto.map((paragraph) => (
      ...
      <section className={styles.index} id="elenco">
        <div className={styles.sectionIntro}>
          ...
          <p>Tre postazioni per allenare i tricipiti da traiettorie diverse. Apri ogni scheda per scoprire funzione, muscoli coinvolti, impostazione, programmazione ed errori comuni.</p>
        </div>
        <div className={styles.grid}>
          {tricepsMachines.map((machine) => (
            <Link key={machine.id} href={`/macchine/tricipiti/${machine.id}`} className={styles.gridCard}>
      ...
      <section className={styles.cta}>
        ...
        <p>Vieni a Revenge Gym e costruisci la tua progressione tricipiti con lo staff.</p>
      ...
      <footer className={styles.footer}>
        ...
        <p>Allenamento · Tricipiti · Ladispoli</p>
