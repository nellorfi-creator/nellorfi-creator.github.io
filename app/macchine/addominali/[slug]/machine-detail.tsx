...
export default function AbsMachineDetail({ machine, prev, next }: Props) {
  ...
  return (
    <main className={styles.page} id="top">
      ...
      <header className={styles.nav}>
        ...
        <nav className={menuOpen ? styles.open : ""} aria-label="Menu scheda macchina addominali">
          <Link className={styles.siteLink} href="/macchine/addominali" onClick={() => setMenuOpen(false)}>
            ← Addominali
          </Link>
      ...
      <div className={styles.body}>
        ...
        <section className={styles.leadBlock}>
          <small>02 · ALLENAMENTO ADDOMINALI</small>
          <h2>
            COME SI USA
            <br />
            <em>IN SALA.</em>
      ...
      <nav className={styles.pager} aria-label="Altre macchine addominali">
        <Link href={`/macchine/addominali/${prev.id}`}>
          <span>← Precedente</span>
          <strong>{prev.name}</strong>
        </Link>
        <Link href="/macchine/addominali" className={styles.pagerHome}>
          Tutti gli addominali
        </Link>
        <Link href={`/macchine/addominali/${next.id}`} className={styles.pagerNext}>
