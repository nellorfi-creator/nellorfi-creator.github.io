"""Spot #4 — Focus Dorso. Montaggio semplice: un'inquadratura veloce per macchina,
...
def render_s1(local):
    ...
    layer = text_layer()
    f = fit_font(ImageDraw.Draw(layer), "NON UNA SOLA TIRATA.", W - 140, 98, 52, 5)
    centered(layer, int(H * 0.44), "NON UNA SOLA TIRATA.", f, WHITE + (255,), 5, 0)
...
def render_s2(local):
    ...
    if local >= 10:
        ...
        layer = text_layer()
        f = font(72, IDX_DEMI)
        centered(layer, int(H * 0.58), "AREA DORSO", f, WHITE + (220,), 14, 0)
        im = composite(im, layer, a1)
    if local >= 28:
        a2 = min((local - 28) / 12.0, 1.0)
        layer = text_layer()
        f = font(156, IDX_HEAVY)
        centered(layer, int(H * 0.66), "10 MACCHINE", f, ORANGE + (255,), 8, 0)
...
def render_s4(local):
    return claim_scene(
        "linear-row",
        [("TIRA DA OGNI ANGOLO.", WHITE, 92, 0.52)],
        local, 100, px=0.48, py=0.44,
    )


def render_s5(local):
    return fast_machine(BLOCK_2, SLOT_FAST, local, idx_offset=5)
...
def render_s6(local):
    return claim_scene(
        ...
        [
            ("LARGHEZZA.", ORANGE, 118, 0.48),
            ("SPESSORE.", WHITE, 118, 0.56),
            ("CONTROLLO.", WHITE, 92, 0.64),
        ],
...
def render_s8(local):
    if local < 55:
        p = local / 54.0
        im = finish(grade(shot("lat-machine-precor", zoom=1.08 + 0.05 * p, px=0.5, py=0.45)))
        ...
        if local >= 4:
            ...
            layer = text_layer()
            f = fit_font(ImageDraw.Draw(layer), "10 POSTAZIONI DORSO.", W - 140, 88, 46, 4)
            centered(layer, int(H * 0.54), "10 POSTAZIONI DORSO.", f, WHITE + (255,), 4, 0)
            im = composite(im, layer, a)
        if local >= 48:
            a2 = min((local - 48) / 8.0, 1.0)
            layer = text_layer()
            f = font(120, IDX_HEAVY)
            centered(layer, int(H * 0.62), "OGNI TIRATA.", f, ORANGE + (255,), 6, 0)
            f2 = font(120, IDX_HEAVY)
            centered(layer, int(H * 0.70), "UN DORSO.", f2, WHITE + (255,), 6, 0)
            im = composite(im, layer, a2)
        return im
...
def main():
    if len(sys.argv) > 1 and sys.argv[1] == "--preview":
        os.makedirs("/tmp/dorso-preview", exist_ok=True)
        for f in [int(x) for x in sys.argv[2].split(",")]:
            render_frame(f).save(f"/tmp/dorso-preview/f{f:05d}.jpg", quality=92)
